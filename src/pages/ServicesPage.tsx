  ...
  return (
    <div className="page-container animate-in">
      ...
      <div className="block md:hidden space-y-3">
        {services.length === 0 ? (
          <Card className="shadow-card border-border/60"><CardContent className="empty-state"><Scissors className="empty-state-icon" /><p className="empty-state-title">Hizmet bulunamadı</p><p className="empty-state-description">İlk hizmetinizi ekleyerek başlayın.</p></CardContent></Card>
        ) : services.map(s => (
          <Card key={s.id} className="shadow-soft border-border/60 hover:shadow-card transition-shadow">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-xl bg-primary/8 flex items-center justify-center"><Scissors className="h-4.5 w-4.5 text-primary" /></div>
                  <div>
                    <p className="font-medium text-sm">{s.name}</p>
                    <div className="flex items-center gap-3 text-xs text-muted-foreground mt-0.5">
                      <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {s.duration} dk</span>
                      <span className="font-semibold text-foreground">₺{Number(s.price).toLocaleString('tr-TR')}</span>
                    </div>
                  </div>
                </div>
            ...
            <TableBody>
              {services.length === 0 ? (
                ...
                <TableRow key={s.id} className="group">
                  <TableCell><div className="flex items-center gap-3"><div className="h-8 w-8 rounded-lg bg-primary/8 flex items-center justify-center"><Scissors className="h-4 w-4 text-primary" /></div><span className="font-medium">{s.name}</span></div></TableCell>
                  <TableCell className="text-muted-foreground">{s.duration} dk</TableCell>
                  <TableCell className="font-semibold">₺{Number(s.price).toLocaleString('tr-TR')}</TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md"><DialogHeader><DialogTitle>{editing ? 'Hizmet Düzenle' : 'Yeni Hizmet'}</DialogTitle></DialogHeader>
