...
export default function StaffPage() {
  ...
  const openAdd = () => { setEditing(null); setForm({ name: '', phone: '', active: true }); setDialogOpen(true); };
  const openEdit = (s: Staff) => { setEditing(s); setForm({ name: s.name, phone: s.phone, active: s.active }); setDialogOpen(true); };
      ...
      <Card>
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {staff.length === 0 ? (
                <TableRow><TableCell colSpan={4} className="text-center py-8 text-muted-foreground">Personel bulunamadı.</TableCell></TableRow>
              ) : staff.map(s => (
                <TableRow key={s.id}>
