  ...
  return (
    <div className="page-container animate-in">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {branches.map(b => (
          <Card key={b.id} className={`shadow-soft border-border/60 hover:shadow-card transition-all ${!b.is_active ? 'opacity-50' : ''}`}>
            <CardContent className="p-5 space-y-3">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-11 w-11 rounded-xl bg-primary/8 flex items-center justify-center"><Building2 className="h-5 w-5 text-primary" /></div>
                  <div><h3 className="font-semibold text-sm">{b.name}</h3><Badge variant={b.is_active ? 'default' : 'secondary'} className="text-[10px] mt-1">{b.is_active ? 'Aktif' : 'Pasif'}</Badge></div>
                </div>
                <div className="flex gap-0.5">
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground" onClick={() => openEdit(b)}><Pencil className="h-3.5 w-3.5" /></Button>
              ...
              <div className="space-y-1.5 text-sm">
                <div className="flex items-center gap-2 text-muted-foreground"><MapPin className="h-3.5 w-3.5 shrink-0" /><span>{b.address || '-'}</span></div>
                <div className="flex items-center gap-2 text-muted-foreground"><Phone className="h-3.5 w-3.5 shrink-0" /><span>{b.phone || '-'}</span></div>
              </div>
              <div className="pt-3 border-t border-border/50 text-xs text-muted-foreground">{getStaffCount(b.id)} personel</div>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      {branches.length === 0 && (
        <Card className="shadow-card border-border/60"><CardContent className="empty-state"><Building2 className="empty-state-icon" /><p className="empty-state-title">Henüz şube eklenmedi</p><p className="empty-state-description">İlk şubenizi ekleyerek başlayın.</p></CardContent></Card>
      )}

      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md"><DialogHeader><DialogTitle>{editing ? 'Şube Düzenle' : 'Yeni Şube'}</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1.5"><Label className="text-xs font-medium">Şube Adı</Label><Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Kadıköy Şubesi" /></div>
            <div className="space-y-1.5"><Label className="text-xs font-medium">Adres</Label><Input value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} placeholder="Kadıköy, İstanbul" /></div>
            <div className="space-y-1.5"><Label className="text-xs font-medium">Telefon</Label><Input value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="0212 555 1234" type="tel" /></div>
